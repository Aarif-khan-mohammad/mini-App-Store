...
import AppItem from '../AppItem/index.jsx'
import './index.css'
...
class AppStore extends Component{
  ...
  render(){
    ...
    const filteredApps = appsList.filter(app => app.category === activeTab)
    ...
    return(
      <div className="AppStore">
        ...
        <div className='search-container'>
          <input type='search' placeholder='Search' onChange={this.handleSearchInput} className='input-bar'/>
          <img src={SEARCH_ICON_URL} className='search-icon'/>
        </div>
        <ul className='tabs-container'>
          {tabsList.map((each)=> <TabItem tabDetails = {each} key = {each.tabId}  clickTabItem={this.clickTabItem}
    isActive={activeTab === each.tabId} />) }
        </ul>

        <ul className='apps-container'>
          { searchResults.map((each)=> <AppItem appDetails = {each} key = {each.appId} />) }
        </ul>
